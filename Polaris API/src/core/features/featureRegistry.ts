...
export const polarisFeatureRegistry: readonly PolarisFeature[] = [
  ...
  ["capability-fabric-1300", "Capability Fabric 1300+", "AUTOMATION", "AVAILABLE", "Catálogo direccionable de más de 1.300 funciones tipadas con ejecución CORE o relay allowlisted.", "Multiplica las posibilidades de Polaris sin recurrir a funciones ficticias ni a ejecución arbitraria."],
  ["web-function-handles", "Function Handles Web", "MULTIPLATFORM", "AVAILABLE", "Expone funciones Web direccionables desde el SDK cliente y el relay seguro.", "Convierte el catálogo en acciones invocables desde el navegador."],
  ["desktop-function-handles", "Function Handles PC", "MULTIPLATFORM", "AVAILABLE", "Expone funciones Desktop direccionables mediante el companion Tauri y el relay existente.", "Permite reutilizar un contrato único desde el programa de PC."],
  ["unified-brain-envelope", "Cerebro unificado", "INTELLIGENCE", "AVAILABLE", "Une seguridad, intención, personalidad, memoria, latencia, experiencia y disponibilidad de dispositivos.", "Hace que Polaris razone sobre una petición como una sola experiencia antes de ejecutar."],
  ["home-assistant-rest-gateway", "Gateway Home Assistant", "MULTIPLATFORM", "PARTIAL", "Conecta Polaris con Home Assistant mediante su API REST cuando se configuran URL y token.", "Integra luces, switches, multimedia, clima y otras entidades sin fingir control directo sobre hardware no conectado."],
  ["home-confirmation-policy", "Confirmación doméstica", "SECURITY", "AVAILABLE", "Exige confirmación para locks, scripts y cambios domésticos de mayor impacto.", "Evita que una automatización del hogar se convierta en una acción silenciosa."],
  ["fabric-preview", "Vista previa universal", "SECURITY", "AVAILABLE", "Previsualiza una función CORE o relay antes de ejecutarla.", "Permite revisar la operación real, payload y riesgo antes de enviarla."],
  ["fabric-client-runtime", "Runtime de capacidades cliente", "MULTIPLATFORM", "AVAILABLE", "Web, PC y Android ejecutan únicamente acciones que sus runtimes ya soportan.", "Mantiene la promesa de funcionalidad real en vez de confundir catálogo con integración."]
].map((entry) => {
  const [id, name, category, status, description, whyItMatters] =
    entry as [string, string, PolarisFeature["category"], PolarisFeature["status"], string, string];
  return { id, name, category, status, description, whyItMatters };
});
